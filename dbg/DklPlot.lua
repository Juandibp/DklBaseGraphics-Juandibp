...
function DklBaseGraphics:pie(x,y,args)
	args = args or {}
	local xlim = xlim or range(x)
	local ylim = ylim or range(y)
	local slices=args.slices or {}
	local lbls=args.lbls or {}
	self:plot_new()
	self:plot_window(xlim,ylim,args)
	local degrees = self:calculateDegress(slices)
	
	local axes = args.axes or true
	local ann = args.axes or self.plt.ann
	local bty = args.bty or "o"
	local diametro = 110
	local ultimoAngulo = 0
	self:title(args)
	for i=1, self:countTable(degrees)
	do
		fill(math.random(0,255),math.random(0,255),math.random(0,255))
		arc(125,90, diametro, diametro, ultimoAngulo, ultimoAngulo+radians(angles[i]))
		ultimoAngulo= ultimoAngulo+radians(angles[i])
	end
...
function DklBaseGraphics:calculateDegress(data)
	local dataTotal=0
	local tempDegree
	angles={}
	for i=1, self:countTable(data)
	do
	...
	for i=1, self:countTable(data)
	do
		tempDegree=(data[i]*360)/dataTotal;
		table.insert(angles, tempDegree)
	end
	return angles
end
	
function DklBaseGraphics:countTable(data)
	local lengthData = 0
